    ...
    , ui(new Ui::LoginDialog), admin_window(nullptr)
{
    ...
    credentials.push_back(login_info("gino","rossi", "m1", "pass", 27, 8, 2003, 1, 23));
    credentials.push_back(login_info("mario","alberti", "m2", "pass", 27, 8, 2002, 1, 25));
    credentials.push_back(login_info("giulio","ferreri", "m3", "pass", 27, 8, 2004, 1, 27));
...
bool LoginDialog::is_valid_email(std::string email){
    ...
    if(!has_at_symbol){
        for(char i : email){
            if(!std::isdigit(i)){ //se trova un carattere che non è una cifra crea messagio di errore
...
void LoginDialog::adminUIConstruct(){
    try{
        ...
        admin_window->setMinimumSize(1000,500);
        lay = new QHBoxLayout();
